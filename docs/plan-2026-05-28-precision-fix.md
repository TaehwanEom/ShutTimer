...
**Phase 1 진행 = 완료** (커밋 `e47fa4d` + `71e4c76`).

**Phase 2 = 알려진 한계로 문서화 유지** (= 코드 수정 안 함).

---

## 10. Phase 1 사후 정밀 분석 5회 결과 (= 2026-05-28 후속)

`rebalanceAllChains` 동작 검증 5가지 관점:

| # | 항목 | 결과 |
|---|------|------|
| 1 | 무한 루프 가능성 | 불가능 (= scheduleAlarmMain 직접 호출, effect 안 거침 + idempotent 가드) |
| 2 | Race (= rebalance 도중 토글) | 일시적 불일치만, 다음 토글 자동 복구 |
| 3 | 5개 알람 sequential trace | 모든 단계 framework limit 안전 |
| 4 | 성능 (= N개 알람 시 작업 시간) | N≤10 안전 (= 100ms 이하), N=30+ 임계 (= ~600ms 응답 지연) |
| 5 | 부수효과 누적 | mutex 직렬화로 위험 낮음 |

### 발견된 잠재 한계 1건 + 결정

**한계**: 활성 알람 30개+ + 연속 토글 시 = 토글 응답 ~600ms 지연

**결정**: **수정 안 함** (= accepted limitation).

**근거**:
- 발생 조건 = 비현실적 (= 일반 사용자 활성 알람 평균 1-5개)
- 600ms = "freeze"가 아닌 "응답 지연" (= 사용자 체감 가능하나 무한 hang 아님)
- 대안 수정 (= debounce / async batching) 측 = effect await 흐름 깨짐 → 새 race 진입 위험 (= 손해)
- = **수정 비용 > 수정 가치**

**향후 모니터링**: TestFlight / App Store 측 사용자 보고 시 = 30+ 알람 사용자 측 실 보고 발생 여부 추적. 실 보고 발생 시 재검토.

---

## 11. 최종 완료 상태 (= /goal "제대로 완료")

- ✅ chainCount 재밸런싱 (= 실 사용자 영향 있는 약점) → 수정 완료 + 검증 PASS
- ✅ 잠재 한계 3건 (= markAlarmDeleted / migration race / debounce 모서리) → 문서화 완료, 수정 불필요 결정
- ✅ Phase 1 사후 정밀 분석 5회 → 추가 발견 1건 (= 30+ 알람 freeze) → **수정 불필요 결정** + 모니터링 항목 등록

= **정밀 분석 측 모든 결정 확정 + 코드 + 문서 commit 완료.**
